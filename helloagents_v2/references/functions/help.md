...
> 注意：可用命令会按项目配置 `helloagents/config.toml` 进行过滤；若配置不存在，先在项目根目录创建 `helloagents/config.toml`（可从 `SKILL_ROOT/assets/templates/config.toml` 复制，默认全命令允许）。被禁用的命令不应展示/不应执行。
