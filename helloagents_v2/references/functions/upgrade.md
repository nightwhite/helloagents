...
步骤2.2 - 扫描知识库:
  使用文件查找工具扫描知识库目录结构与文件列表
  输出: 目录结构 + 文件清单（结构化列表即可）
```
...
步骤5.2 - 创建目录结构:
  创建: modules/, archive/, plan/ 等标准目录结构
...
步骤5.4 - 写入文件:
  ...
  方式B - 批量写入（大量文件时）:
    1. AI 生成操作计划（JSON 结构即可，不要求落盘）
    2. 按计划逐项执行 mkdir/write/rename/delete
    3. 检查执行结果
...
> 📌 规则引用: 路径基准、文件操作、错误恢复规则见 references/rules/tools.md

### 核心操作

```yaml
扫描目录:
  输出: 文件列表和目录结构（结构化列表即可）
  用途: 获取知识库当前状态

创建目录:
  输出: 创建结果（created/existed 列表，可选）
  用途: 初始化标准目录结构
