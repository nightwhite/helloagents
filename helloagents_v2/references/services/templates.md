...
> 📌 规则引用: 执行自检报告机制详见 references/rules/tools.md
...
被引用:
  ...
  - 方案设计阶段（方案包模板使用）
  - ~exec 命令（方案包验证）
  - create_package 能力（模板加载）
  - migrate_package 能力（_index.md 模板）
  - validate_package 能力（章节验证）
...
引用:
  ...
  - G7 方案包类型
  - G7 任务状态符号
  - references/rules/tools.md（文件/模板操作规范与自检报告）
