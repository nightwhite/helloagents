...
**实现（无脚本）:**
```yaml
创建方案包: 在 plan/ 下创建 YYYYMMDDHHMM_{feature}/ 并写入 proposal.md/tasks.md 模板
项目规模统计（可选）: 统计文件数/行数/模块数
```
...
检查项:
  - 方案包目录存在
  - proposal.md / tasks.md 存在且包含必需章节
  - 设置 CREATED_PACKAGE 为方案包路径
```
...
```yaml
步骤:
  1. 执行方案包格式检查（proposal.md / tasks.md / 目录命名 / 状态符号）
  2. 检查验收结果
