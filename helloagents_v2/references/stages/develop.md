...
```yaml
执行规则:
  - 按 tasks.md 顺序遍历任务，但必须“按状态执行/跳过”（支持续跑，不重复做已完成任务）

预处理（强制）:
  - 若 tasks.md 中所有任务均为 [√] 或 [-]（无 [ ]/[X]/[?]）:
    - 视为“已完成未归档”
    - 跳过本步骤7-13（不再执行代码改动/测试/KB等）
    - 直接进入步骤14归档，并在完成总结中说明“任务已完成，已补归档”

任务按状态处理:
  - [√] 已完成: 跳过，不重复执行
  - [-] 已跳过: 跳过（除非用户明确要求重做/重试）
  - [ ] 待执行: 正常执行
  - [?] 待确认: 必须询问用户澄清后再决定执行或跳过（AUTO_FULL 下也不得静默猜测）
  - [X] 失败: 交互确认是否重试；默认建议重试一次（用户拒绝则标记为 [-] 并写备注）
...
**实现（无脚本）:** 迁移方案包到 `archive/YYYY-MM/` 并更新索引
